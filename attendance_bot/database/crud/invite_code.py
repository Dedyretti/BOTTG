...
def _utc_now() -> datetime:
    """Возвращает текущее UTC время без timezone."""
...
async def get_invite_code_for_employee(
    ...
    """Получает инвайт-код по UUID и employee_id."""
    code_uuid = _to_uuid(code)
...
async def get_active_invite_code(
    ...
    """Получает активный инвайт-код для сотрудника."""
    now = _utc_now()
...
async def mark_invite_code_used(
    ...
    invite_code = await get_invite_code_by_code(session, code)
...
async def deactivate_old_invite_codes(
    ...
    """Деактивирует все старые неиспользованные коды сотрудника."""
    result = await session.execute(
        select(InviteCode).where(
            InviteCode.employee_id == employee_id,
            InviteCode.is_used.is_(False),
...
async def create_invite_code(
    ...
    """Создаёт новый инвайт-код для сотрудника."""
